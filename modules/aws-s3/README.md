...
| <a name="input_create_bucket_policy"></a> [create\_bucket\_policy](#input\_create\_bucket\_policy) | Whether to create a bucket policy to enforce HTTPS only and replication permissions. If false, the module will not create a bucket policy and will not enforce HTTPS or replication permissions. | `bool` | `true` | no |

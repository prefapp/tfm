variable "config" {
  ...
  validation {
    condition     = contains(["branch", "tag", "push"], var.config.target)
    error_message = "config.target must be one of: 'branch', 'tag', 'push'."
  }

  validation {
    condition     = contains(["disabled", "evaluate", "active"], var.config.enforcement)
    error_message = "config.enforcement must be one of: 'disabled', 'evaluate', 'active'."
  }

...
resource "github_organization_ruleset" "this" {
  ...
  dynamic "conditions" {
    for_each = var.config.conditions != null ? [var.config.conditions] : []
    content {
      # ref_name is only valid for branch and tag targets — not for push.
      dynamic "ref_name" {
        for_each = var.config.target != "push" && conditions.value.ref_name != null ? [conditions.value.ref_name] : []
        content {
  ...
  rules {
    ...
    creation                = contains(keys(local.rules_by_type), "creation") ? true : null
    deletion                = contains(keys(local.rules_by_type), "deletion") ? true : null
    update                  = contains(keys(local.rules_by_type), "update") ? true : null
    non_fast_forward        = contains(keys(local.rules_by_type), "non_fast_forward") ? true : null
    required_linear_history = contains(keys(local.rules_by_type), "required_linear_history") ? true : null
    required_signatures     = contains(keys(local.rules_by_type), "required_signatures") ? true : null

    dynamic "pull_request" {
      for_each = lookup(local.rules_by_type, "pull_request", null) != null ? [local.rules_by_type["pull_request"]] : []
      content {
    ...
    dynamic "required_status_checks" {
      for_each = lookup(local.rules_by_type, "required_status_checks", null) != null ? [local.rules_by_type["required_status_checks"]] : []
      content {
    ...
    dynamic "commit_message_pattern" {
      for_each = lookup(local.rules_by_type, "commit_message_pattern", null) != null ? [local.rules_by_type["commit_message_pattern"]] : []
      content {
    ...
    dynamic "commit_author_email_pattern" {
      for_each = lookup(local.rules_by_type, "commit_author_email_pattern", null) != null ? [local.rules_by_type["commit_author_email_pattern"]] : []
      content {
    ...
    dynamic "committer_email_pattern" {
      for_each = lookup(local.rules_by_type, "committer_email_pattern", null) != null ? [local.rules_by_type["committer_email_pattern"]] : []
      content {
    ...
    dynamic "branch_name_pattern" {
      for_each = lookup(local.rules_by_type, "branch_name_pattern", null) != null ? [local.rules_by_type["branch_name_pattern"]] : []
      content {
    ...
    dynamic "tag_name_pattern" {
      for_each = lookup(local.rules_by_type, "tag_name_pattern", null) != null ? [local.rules_by_type["tag_name_pattern"]] : []
      content {
    ...
    dynamic "copilot_code_review" {
      for_each = lookup(local.rules_by_type, "copilot_code_review", null) != null ? [local.rules_by_type["copilot_code_review"]] : []
      content {
    ...
    dynamic "file_path_restriction" {
      for_each = lookup(local.rules_by_type, "file_path_restriction", null) != null ? [local.rules_by_type["file_path_restriction"]] : []
      content {
    ...
    dynamic "file_extension_restriction" {
      for_each = lookup(local.rules_by_type, "file_extension_restriction", null) != null ? [local.rules_by_type["file_extension_restriction"]] : []
      content {
    ...
    dynamic "max_file_size" {
      for_each = lookup(local.rules_by_type, "max_file_size", null) != null ? [local.rules_by_type["max_file_size"]] : []
      content {
    ...
    dynamic "max_file_path_length" {
      for_each = lookup(local.rules_by_type, "max_file_path_length", null) != null ? [local.rules_by_type["max_file_path_length"]] : []
      content {

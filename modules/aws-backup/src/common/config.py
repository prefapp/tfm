...
def load_config() -> Config:
    """
    Loads configuration for backup replication from environment variables and JSON.
    Returns:
    ...
    for account_id, entry in parsed.items():
        ...
        destinations[account_id] = Destination(
            ...
            regions=regions,
            iam_role_arn=entry["iam_role_arn"],
            delete_after_days=entry.get("delete_after_days", 30)
        )

locals {
  has_cross_account_copy_default = var.copy_action_default_values.destination_account_id != null && var.copy_action_default_values.destination_account_id != data.aws_caller_identity.current.account_id ? true : false
  # Returns true if there is any copy_action with destination_vault_arn whose account_id is different from the current one

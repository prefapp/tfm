...
locals {
  ...
  s3_bucket_id       = local.using_existing_s3_bucket ? regexall("^arn:aws:s3:::(.+)$", var.s3_bucket_arn)[0] : (length(aws_s3_bucket.cloudtrail) > 0 ? aws_s3_bucket.cloudtrail[0].id : null)

...
resource "azurerm_postgresql_flexible_server" "this" {
  ...
  delegated_subnet_id            = var.public_network_access_enabled == false ? : try(data.azurerm_subnet.subnet[0].id, null) : null
  private_dns_zone_id            = var.public_network_access_enabled == false ? : try(data.azurerm_private_dns_zone.dns_private_zone[0].id, null) : null

...
locals {
  ...
  virtual_network_name = coalesce(var.vnet.name, data.azurerm_resources.vnet[0].name)
  resource_group_name  = coalesce(var.vnet.resource_group_name, data.azurerm_resources.vnet[0].resource_group_name)
}

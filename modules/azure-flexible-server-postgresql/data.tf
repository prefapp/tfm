...
data "azurerm_key_vault_secret" "administrator_password" {
  ...
  key_vault_id = length(try(data.azurerm_key_vault.key_vault, [])) > 0 ? try(data.azurerm_key_vault.key_vault[0].id, null) : (
                 length(try(data.azurerm_resources.key_vault, [])) > 0 && length(try(data.azurerm_resources.key_vault[0].resources, [])) > 0 ?
                 try(data.azurerm_resources.key_vault[0].resources[0].id, null) : null)

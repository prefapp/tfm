...
locals {
  ...
  resource_group_name  = var.public_network_access_enabled == false ? try(coalesce(var.vnet.resource_group_name, local.resource_group_from_data), null) : null
  virtual_network_name = var.public_network_access_enabled == false ? try(coalesce(var.vnet.name, local.vnet_from_data), null) : null
}

...
| <a name="input_key_vault"></a> [key\_vault](#input\_key\_vault) | Locate the Key Vault either via `name` + `resource_group_name` together, or via non-empty `tags` used as `required_tags` on `azurerm_resources`. | <pre>object({<br/>    name                = optional(string)<br/>    resource_group_name = optional(string)<br/>    tags                = optional(map(string))<br/>  })</pre> | `{}` | no |

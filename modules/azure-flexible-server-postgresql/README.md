...
| <a name="input_key_vault"></a> [key\_vault](#input\_key\_vault) | Locate the Key Vault via `name` + `resource_group_name`, or via `required_tags` on `azurerm_resources` when `tags` is non-empty. | <pre>object({<br/>    name                = optional(string)<br/>    resource_group_name = optional(string)<br/>    tags                = optional(map(string))<br/>  })</pre> | `{}` | no |

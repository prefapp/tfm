...
locals {
  ...
  names = var.provider.provider_name == "AZURE" ? [for secret in data.azurerm_key_vault_secrets.key_vault_secrets.secrets : secret.name if can(regex("test-${var.provider.provider_name}-.*", secret.name))] : []
}

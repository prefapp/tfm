...
| <a name="input_budget"></a> [budget](#input\_budget) | Budget alert configuration(s). Accepts either a legacy single object or a map of objects keyed by logical name. | `any` | `{}` | no |
...
| <a name="input_quota_alert"></a> [quota\_alert](#input\_quota\_alert) | Quota alert configuration(s). Accepts either a legacy single object or a map of objects keyed by logical name. | `any` | `{}` | no |

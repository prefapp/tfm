...
resource "azurerm_application_gateway" "application_gateway" {
  ...
  dynamic "ssl_profile" {
    ...
    content {
      ...
      trusted_client_certificate_names     = [for certName, certData in data.external.list_cert_files.result : certName if strcontains(jsondecode(certData).caDir, ssl_profile.value.ca_certs_origin.github_directory)]

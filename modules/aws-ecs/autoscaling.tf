...
resource "aws_appautoscaling_target" "ecs" {
  for_each = {
    ...
    if v.autoscaling_enabled && try(v.scale.halt, null) == null
  }
  ...
  resource_id        = var.create_cluster ? "service/${aws_ecs_cluster.this[0].name}/${each.key}" : "service/${var.cluster_name}/${each.key}"

...
resource "aws_iam_role" "iam_role_oidc_new" {

  count = var.create_alb_ingress_iam == true ? 1 : 0
...
resource "aws_iam_policy" "iam_policy_alb_new" {
...
resource "aws_iam_role_policy_attachment" "iam_role_scm_attachment_new" {
  ...
  role       = aws_iam_role.iam_role_oidc_new.name
  policy_arn = aws_iam_policy.iam_policy_alb_new.arn
}

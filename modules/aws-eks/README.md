...
| <a name="cluster_security_group_additional_rules"></a> [cluster\_security\_group\_additional\_rules](#input\_cluster\_security\_group\_additional\_rules) | Additional rules for the cluster security group | <pre>map(object({<br><br>    description = string<br><br>    protocol = string<br><br>    from_port = number<br><br>    to_port = number<br><br>    type = string<br><br>    source_node_security_group = optional(bool)<br><br>  }))</pre> | `{}` | No |

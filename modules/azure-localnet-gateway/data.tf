...
data "azurerm_key_vault_secret" "s2s" {
	for_each = { for idx, s in var.localnet : idx => s if try(s.keyvault_secret_name, null) != null && try(s.keyvault_vault_name, null) != null && try(s.keyvault_vault_rg, null) != null }
	name         = each.value.keyvault_secret_name
	key_vault_id = data.azurerm_key_vault.s2s[each.key].id
}

data "azurerm_key_vault" "s2s" {
	for_each = { for idx, s in var.localnet : idx => s if try(s.keyvault_vault_name, null) != null && try(s.keyvault_vault_rg, null) != null }
